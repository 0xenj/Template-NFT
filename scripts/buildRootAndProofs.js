...
async function main() {
  console.log(
    ...
    "\x1b[33m",
    "Building " +
      foundryTestDataFileName +
      "." +
      foundryTestFileExtension +
      "...",
    "\x1b[0m"
  );
  ...
  for (let i = 11; i <= 20; i++) {
    const proofs = await functions.returnBuildProof(
      `0x00000000000000000000000000000000000000${i}`,
      addressesWhiteList
    );
...
${solidityProofsWhiteList} }
  ...
  console.log(
    "\x1b[32m",
    `${foundryTestDataFileName}.${foundryTestFileExtension} file created in foundry ${foundryTestFolderPath} folder`
  );
  console.log(
    "\x1b[0m",
    ` with ${addressesWhiteList.length} addresses in WhiteList`
  );
