...
const help_startMinting = async (
  ...
  const input = await contract
    .connect(sender)
    .populateTransaction.startMinting(
      startPublicTimestamp,
      startWhiteListTimestamp
    );
...
const help_isWhitelistValid = async (
  contract,
  sender,
  allAddresses,
  isValid
  ...
  const isWhitelistValid = await contract
    .connect(sender)
    .isWhitelistValid(proofs);
...
const help_tokenURI = async (contract, sender, tokenId, error) => {
  const input = await contract
    .connect(sender)
    .populateTransaction.tokenURI(tokenId);
  await checkRawTxnResult(input, sender, error);
};
...
const help_mint = async (contract, sender, quantity, value, error) => {
  ...
  const input = await contract
    .connect(sender)
    .populateTransaction.mint(quantity, {
      value,
    });
  await checkRawTxnResult(input, sender, error);
  if (!error) {
    const finalTotalSupply = await contract.totalSupply();
    expect(ethers.BigNumber.from(totalSupply).add(quantity)).to.equal(
      ethers.BigNumber.from(finalTotalSupply)
    );
    if (quantity === 1) {
...
const help_mintWhiteList = async (
  contract,
  sender,
  quantity,
  proofs,
  ...
  const input = await contract
    .connect(sender)
    .populateTransaction.mintWhiteList(quantity, proofs, {
      value,
    });
  await checkRawTxnResult(input, sender, error);
...
const help_withdrawEther = async (
  contract,
  sender,
  expectedEtherToWithdraw,
  error
) => {
  ...
  const input = await contract
    .connect(sender)
    .populateTransaction.withdrawEther();
  await checkRawTxnResult(input, sender, error);
  ...
  if (!error) {
    ...
    const txGasCost = senderBalanceAfter
      .sub(senderBalanceStart)
      .sub(expectedEtherToWithdraw);
    expect(contractBalanceAfter).to.equal(0);
    expect(ethers.utils.formatEther(senderBalanceAfter)).to.equal(
      ethers.utils.formatEther(
        senderBalanceStart.add(expectedEtherToWithdraw).add(txGasCost)
      )
    );
  }
};
